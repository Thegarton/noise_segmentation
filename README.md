...
then builds a temporary numbered JPEG folder containing only those requested frames and passes that folder to
`start_session`, so the model does not decode/cache the full source video. If you need the old full-video behavior,
pass `--sam3-wrapper-arg=--use-original-video-resource`.
