    ...
    *,
    ...
    if sign_type_classifier is not None:
        if sign_type_class_mapping is None:
            raise ValueError("sign_type_class_mapping is required when the sign classifier is enabled")
...
def resolve_sign_type_class_mapping(class_to_id: dict[str, int]) -> dict[str, int]:
    ...
    if missing:
        raise ValueError(f"Sign classifier output classes are missing from classes yaml: {missing}")
...
def apply_sign_type_classification(
    *,
    ...
    if min_mask_size <= 0:
        raise ValueError(f"min_mask_size must be positive for sign classification, got {min_mask_size}")
    ...
    missing_mapping = [label for label in SIGN_TYPE_LABELS if label not in class_mapping]
    if missing_mapping:
    ...
    retained_instances = [item for item in instances if item.label not in SIGN_TYPE_LABELS]
    ...
    for candidate, decision in zip(candidates, decisions):
        ...
        if predicted_label not in expected_classifier_classes:
            raise ValueError(f"Sign classifier returned unsupported label {predicted_label!r}")
        ...
        sam3_scores = tuple(
            (label, float(candidate.class_scores[label]))
        ...
        routed_instances.append(
            Sam3Instance(
                ...
                box=None if canonical.box is None else np.asarray(canonical.box, dtype=np.float32),
...
def validate_outputs(frame_out: Path) -> None:
    ...
    if sign_confidences.shape != (masks.shape[0],) or sign_accepted.shape != (masks.shape[0],):
...
def sam3_single_image_folder(
    ...
    validate_probability(sign_classifier_min_confidence, name="sign_classifier_min_confidence")
    validate_probability(sign_classifier_min_margin, name="sign_classifier_min_margin")
    validate_probability(sign_classifier_cluster_iou, name="sign_classifier_cluster_iou")
    validate_probability(
        sign_classifier_cluster_containment,
        name="sign_classifier_cluster_containment",
    )
    if not np.isfinite(sign_classifier_context_scale) or sign_classifier_context_scale < 1.0:
        raise ValueError(
