...
class ActorAutoLabeler:
    ...
    def run(self, sample: SequenceSample) -> list[LabelInstance]:
        current_proposals = self.proposal_model.predict(sample.current)
        temporal_proposals = {
            "past": [self.proposal_model.predict(frame) for frame in sample.past[-2:]],
            "future": [self.proposal_model.predict(frame) for frame in sample.future[:2]],
        }
        ...
        for det in tracked:
            proposal: ActorProposal = det["proposal"]
