"""Minimal SAM3.1 Multiplex adapter reused for all prepared sign images."""
...
class Sam3SignDetector:
    """Load one predictor and run all configured prompts for every image."""
...
def _extract_arrays(outputs: Any, *, height: int, width: int) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]:
    ...
    masks = _to_numpy(_first_present(outputs, MASK_KEYS))
    scores = _to_numpy(_first_present(outputs, SCORE_KEYS))
    boxes = _to_numpy(_first_present(outputs, BOX_KEYS))
    if masks is None:
